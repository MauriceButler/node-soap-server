...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest, callback){
	...
	for(var operationRequestName in operationRequest){
		...
		operationArguments.push(function(error, operationResponse){
			if(error){
				return callback(error);
			}
			var response = {},
				responseName = operationName + 'Response';
		...
		*/
		...
		operation.exec.apply(operation, operationArguments);
	}
...
SoapServer.prototype.handleRequest = function(req, res){
	try{
		this.handleSoapRequest(req, function(error, response){
	...
	if (req.method == 'POST') {
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				This.handleOperationRequest(service, soapRequest, function(error, soapResponse){
					...
					if(error){
						...
						responseXml += '		<soap:faultcode>' + error.type + '</soap:faultcode>\n';
						responseXml += '		<soap:faultstring>' + error.message + '</soap:faultstring>\n';
						responseXml += '	</soap:Fault>\n';
					}
					...
					callback.apply(null, [error, responseXml]);
